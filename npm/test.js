...
async function runTests() {
  ...
  try {
    ...
    // [Security Hardening Test 10]: State file is a symlink -> MUST return false
    const symStatePath = path.join(os.tmpdir(), 'test_vulkan_symlink.json');
    try {
      fs.symlinkSync(tempStatePath, symStatePath);
      const symDoc = new Doctor(symStatePath);
      assert.strictEqual(symDoc.quickProbe(), false, "Symlinked state file MUST return false");
      fs.unlinkSync(symStatePath);
    } catch (_) {
      // In unprivileged Windows environments, symlink creation may require privilege
    }

    // [Security Hardening Test 11]: World-writable state file -> MUST return false
    if (process.platform !== 'win32') {
      fs.chmodSync(tempStatePath, 0o666);
      assert.strictEqual(validDoc.quickProbe(), false, "World-writable state file MUST return false");
