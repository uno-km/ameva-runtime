...
class Doctor {
  ...
  quickProbe() {
    ...
    try {
      ...
      if (process.platform !== 'win32') {
        // Reject world-writable state files
        if ((lstat.mode & 0o002) !== 0) {
          return false;
        }
        // Verify file owner is current UID
        if (typeof process.getuid === 'function' && lstat.uid !== process.getuid()) {
          return false;
        }
        // Verify parent directory permissions and type
        const parentDir = path.dirname(this.statePath);
        if (fs.existsSync(parentDir)) {
          const parentStat = fs.lstatSync(parentDir);
          if (parentStat.isSymbolicLink() || (parentStat.mode & 0o002) !== 0) {
            return false;
          }
        }
      }

      // Safe read preventing symlink race (O_NOFOLLOW if supported)
      let content;
      if (process.platform !== 'win32' && fs.constants.O_NOFOLLOW) {
        const fd = fs.openSync(this.statePath, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW);
        try {
        ...
        content = fs.readFileSync(this.statePath, 'utf-8');
      }
