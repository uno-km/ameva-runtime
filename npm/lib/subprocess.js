 ...
 * - Absolute path only
 * - No NUL bytes
 * - Symbolic links strictly forbidden (anti-TOCTOU hardening)
 * - Realpath resolution and regular file check
 ...
 * @returns {Promise<string>} Canonical realpath of the executable
 */
async function validateExecutable(executable) {
  ...
  let originalStat;
  try {
    originalStat = await fs.promises.lstat(normalized);
  } catch (err) {
    throw new Error(`Executable file not found: ${normalized} (${err.message})`);
  }

  if (originalStat.isSymbolicLink()) {
    throw new Error(`Executable symbolic links are not permitted: ${normalized}`);
  }
  ...
  try {
    ...
    throw new Error(`Failed to resolve canonical path for executable: ${normalized} (${err.message})`);
  }

  const finalStat = await fs.promises.stat(realPath);
  if (!finalStat.isFile()) {
    throw new Error(`Executable must resolve to a regular file: ${realPath}`);
  }

  if (process.platform === 'win32') {
    const ext = path.extname(realPath).toLowerCase();
    if (ext !== '.exe') {
      throw new Error(`On Windows, only direct .exe executables are permitted with shell:false. Got: '${ext}' for ${realPath}`);
    }
  } else {
    try {
      await fs.promises.access(realPath, fs.constants.X_OK);
    } catch (err) {
      throw new Error(`Executable lacks execution permission (X_OK): ${realPath}`);
    }
  }
