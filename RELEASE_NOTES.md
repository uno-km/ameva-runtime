...
- **Node.js Package**: `@ameva/runtime v2.6.3`
- **Python Distribution**: `2.6.3`
...
#### Security & Integrity Patches
- **P0 Archive Path Traversal Protection**: Hardened `python/ameva_runtime/installer.py` with `safe_extract_tar()`. Rejects parent directory traversal (`../`), absolute paths, and archive links (symlinks and hardlinks) before extraction.
- **P1 Mandatory SHA-256 Checksum Enforcement**: Enforced pinned 64-character hexadecimal SHA-256 verification (`verify_sha256()`) prior to any archive extraction or binary deployment. Immediate file unlinking on digest mismatch; unauthenticated asset provisioning rejected.
- **P1 Executable Validation & Anti-TOCTOU**: Hardened `lib/subprocess.js` `validateExecutable()`. Strictly rejects symbolic links via `lstat()`, resolves canonical `realpath()`, enforces regular file status and execution permissions, and passes canonical path to `spawn()`.
- **P2 Native Addon Environment Override Security**: Hardened `lib/native_bridge.js`. Banned `AMEVA_ALLOW_NATIVE_OVERRIDE` in `production`, rejected symbolic links and world-writable files/parent directories, and mandated cryptographic SHA-256 digest validation (`AMEVA_NATIVE_SHA256`).
- **P2 Doctor State Verification Hardening**: Hardened `lib/doctor.js`. Enforced 0600 file and 0700 parent directory modes, rejected symlinked and world-writable state files, verified process UID ownership, and applied `O_NOFOLLOW` safe descriptors.
