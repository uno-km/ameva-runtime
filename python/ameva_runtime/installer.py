...
def get_release_base_url() -> str:
    ...
    2. Versioned release tag matching current package version: v{__version__} (if release exists)
    3. Canonical release endpoint containing SOTA GPU asset bundles: v2.7.1
    """
...
LOCAL_BIN = PREFIX_BIN  # Backward compatibility alias
LOCAL_LIB = PREFIX_LIB  # Backward compatibility alias
AMEVA_CACHE = Path(os.environ.get("XDG_CACHE_HOME", HOME / ".cache")) / "ameva"
AMEVA_MODELS = AMEVA_CACHE / "models"
AMEVA_STAGING = AMEVA_CACHE / ".staging"
...
def safe_extract_tar(
    ...
    with tarfile.open(archive_path, "r:gz") as tar:
        for member in tar.getmembers():
            ...
            # Reject hard links unconditionally
            if member.islnk():
                raise AmevaLlamaUnsafeArchiveError(
                    f"Archive links are not permitted: {member.name}"
                )
            # Allow safe internal symlinks; strictly reject escaping symlinks
            if member.issym():
                link_dest = (root / Path(member.name).parent / member.linkname).resolve()
                try:
                    link_dest.relative_to(root)
                except ValueError:
                    raise AmevaLlamaUnsafeArchiveError(
                        f"Archive links are not permitted: {member.name}"
                    )
...
NATIVE_ASSETS: Dict[str, AssetSpec] = {
    ...
    "llamacpp": AssetSpec(
        name="llamacpp",
        filename="llama-cli-vulkan-android-arm64.tar.gz",
        download_sha256="6775373f224ad6427225ed943c8bc38f065d14e6fe89dd56cf9b3bf2b7a56022",
        binary_relpath="bin/llama-cli",
        canonical_name="llama-cli",
        description="llama.cpp ARM64 Vulkan GPU Acceleration Native Engine",
    ),
    "bitnet": AssetSpec(
        name="bitnet",
        filename="termux-bitnet-v1.4.0-android-aarch64.tar.gz",
        download_sha256="6466c1d22917612a8c6b7bde29cce095c88fd0c736fe3e23c218695f5c675ea2",
        binary_relpath="termux-bitnet-cli",
        canonical_name="termux-bitnet-cli",
        description="BitNet 1.58-bit ARM64 Vulkan GPU Acceleration Native Engine",
    ),
}
...
class NativeAssetManager:
    ...
    def _verify_existing_bundle_or_raise(self, spec: AssetSpec) -> None:
        ...
        if actual_binary != expected_binary:
            # If canonical binary was deployed as an atomic file copy rather than symlink,
            # verify that its cryptographic content hash matches the release binary exactly.
            if _calc_file_sha256(actual_binary) != _calc_file_sha256(expected_binary):
                raise AmevaLlamaVerificationError(
    ...
    def _provision_asset_locked(self, spec: AssetSpec) -> Any:
        ...
        if spec.target_root:
            ...
            current_link = AMEVA_CURRENT / spec.name
            canonical_bin = PREFIX_BIN / spec.canonical_name
            manifest_store = AMEVA_MANIFESTS

        PREFIX_BIN.mkdir(parents=True, exist_ok=True)
        PREFIX_LIB.mkdir(parents=True, exist_ok=True)
        AMEVA_STAGING.mkdir(parents=True, exist_ok=True)
        ...
        if is_installed and not (self.force or self.force_repair):
            if spec.name in ("llamacpp", "bitnet"):
                try:
        ...
        with tempfile.TemporaryDirectory(prefix=f".staging-{spec.name}-", dir=str(AMEVA_STAGING)) as temp_work_dir:
            ...
            # Verify and chmod any bundled shared libraries
            ...
            if os.name != "nt":
                try:
                    ...
                    prefix_lib = Path(os.environ.get("PREFIX") or "/data/data/com.termux/files/usr") / "lib"
                    staged_lib_dirs = [
                        str(staging_release),
                    ...
                    # Auto-inject mobile Vulkan HAL shim for Samsung GOS compatibility
                    shim_candidates = [
                        prefix_lib / "libegl_shim.so",
                        Path(__file__).resolve().parent / "vulkan" / "lib" / "libegl_shim.so",
                    ]
                    for sc in shim_candidates:
                        if sc.is_file():
                            smoke_env["LD_PRELOAD"] = str(sc)
                            break
                    ...
                    if smoke_proc.returncode != 0:
                        smoke_proc = subprocess.run(
                            [str(staged_primary), "--help"],
                            capture_output=True,
                            text=True,
                            timeout=10,
                            env=smoke_env,
                        )
                    # Isolated fallback pass in case of Bionic symbol collision
                    if smoke_proc.returncode != 0 and "Usage" not in (smoke_proc.stdout + smoke_proc.stderr):
                        isolated_env = os.environ.copy()
                        isolated_env.pop("LD_LIBRARY_PATH", None)
                        isolated_env.pop("LD_PRELOAD", None)
                        smoke_proc = subprocess.run(
                            [str(staged_primary), "--help"],
                            capture_output=True,
                            text=True,
                    ...
                    print(f"    -> [PASS] Smoke test verified: {staged_primary.name}")
            ...
            # 9. Direct deployment of shared libraries to $PREFIX/lib
            PREFIX_LIB.mkdir(parents=True, exist_ok=True)
            for so_file in release_dir.rglob("*.so*"):
                if so_file.is_file() and not so_file.is_symlink():
                    target_so = PREFIX_LIB / so_file.name
                    tmp_so = PREFIX_LIB / f".tmp_{so_file.name}"
                    shutil.copy2(str(so_file), str(tmp_so))
                    if os.name != "nt":
                        tmp_so.chmod(0o755)
                    os.replace(tmp_so, target_so)

            # 10. Direct atomic deployment of canonical binary to $PREFIX/bin
            if spec.canonical_name:
                PREFIX_BIN.mkdir(parents=True, exist_ok=True)
                tmp_bin = PREFIX_BIN / f".tmp_{canonical_bin.name}"
                if tmp_bin.exists() or tmp_bin.is_symlink():
                    tmp_bin.unlink(missing_ok=True)
                src_bin = release_dir / spec.binary_relpath
                shutil.copy2(str(src_bin), str(tmp_bin))
                if os.name != "nt":
                    tmp_bin.chmod(0o755)
                os.replace(tmp_bin, canonical_bin)

                if spec.name == "llamacpp":
                    for extra_bin in ("llama-server", "llama-completion"):
                        extra_src = release_dir / "bin" / extra_bin
                        if extra_src.is_file():
                            tmp_extra = PREFIX_BIN / f".tmp_{extra_bin}"
                            shutil.copy2(str(extra_src), str(tmp_extra))
                            if os.name != "nt":
                                tmp_extra.chmod(0o755)
                            os.replace(tmp_extra, PREFIX_BIN / extra_bin)
                    for alias in ("termux-llama", "termux-llama-cli"):
                        alias_path = PREFIX_BIN / alias
                        if alias_path.exists() or alias_path.is_symlink():
                            alias_path.unlink(missing_ok=True)
                        if os.name != "nt":
                            alias_path.symlink_to(canonical_bin.name)

                if spec.name == "bitnet":
                    for alias in ("termux-bitnet",):
                        alias_path = PREFIX_BIN / alias
                        if alias_path.exists() or alias_path.is_symlink():
                            alias_path.unlink(missing_ok=True)
                        if os.name != "nt":
    ...
    def _check_path_environment(self) -> None:
        """Notifies user if PREFIX_BIN is absent from current PATH."""
        cur_path = os.environ.get("PATH", "")
        prefix_bin_str = str(PREFIX_BIN)
        if prefix_bin_str not in cur_path and os.name != "nt":
            print(f"\n[NOTICE] {prefix_bin_str} is not in your current PATH.")
            print("  To make all installed tools globally accessible, run:")
            print(f'  echo \'export PATH="{prefix_bin_str}:$PATH"\' >> ~/.bashrc && source ~/.bashrc')
