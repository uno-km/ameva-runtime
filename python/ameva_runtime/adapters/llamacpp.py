...
class LlamaCppAdapter(BaseAdapter):
    ...
    def get_execution_environment(
        ...
        **kwargs: Any,
        ...
        Strictly isolates engine libraries and system Vulkan drivers without Termux symbol pollution.
        """
        env = dict(base_env or os.environ)
        home = Path(os.environ.get("HOME", os.path.expanduser("~")))
        std_llama_lib = home / ".local" / "share" / "ameva" / "current" / "llamacpp" / "lib"
        legacy_llama_lib = home / ".termux-llama" / "current" / "lib"

        lib_paths = []
        if std_llama_lib.is_dir():
            lib_paths.append(str(std_llama_lib))
        elif legacy_llama_lib.is_dir():
            lib_paths.append(str(legacy_llama_lib))

        sys_lib64 = Path("/system/lib64")
        if sys_lib64.is_dir():
            lib_paths.append(str(sys_lib64))

        env["LD_LIBRARY_PATH"] = ":".join(lib_paths)
        # Ensure GGML_VULKAN_SKIP_CHECKS is never present
    ...
    @classmethod
    def resolve_binary_path(cls, binary_name: str = "llama-cli") -> str:
        """Resolves authoritative binary path strictly without PATH or fallback search.

        Authoritative SSOT location: $PREFIX/bin/<binary_name>
        - Manifest: ~/.local/share/ameva/manifests/llamacpp.json
        ...
        """
        try:
            return str(resolve_authoritative_binary(binary_name, "llamacpp"))
        except (AmevaLlamaAssetMissingError, AmevaLlamaVerificationError) as err:
            home = Path(os.environ.get("HOME", os.path.expanduser("~")))
            legacy_bin = home / ".termux-llama" / "current" / "bin" / binary_name
            if legacy_bin.is_file():
                legacy_manifest = home / ".termux-llama" / "current" / "manifest.json"
                if not legacy_manifest.is_file():
                    legacy_manifest = home / ".termux-llama" / "manifests" / "llamacpp.json"
                if legacy_manifest.is_file():
                    return str(resolve_authoritative_binary(binary_name, "llamacpp", manifest_path=legacy_manifest))
            raise err

    @classmethod
    def resolve_completion_binary_path(cls) -> str:
        """Resolves the single authoritative llama-completion binary path strictly."""
        return cls.resolve_binary_path(binary_name="llama-completion")
    ...
    def build_cli_args(
        ...
        if Path(executable).name == "llama-completion":
            if "-no-cnv" not in cmd and "--no-conversation" not in cmd:
                cmd.append("-no-cnv")
            if "-fa" not in cmd and "--flash-attn" not in cmd:
                cmd.extend(["-fa", "0"])
